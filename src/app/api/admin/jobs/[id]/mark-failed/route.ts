...
export async function POST(
  ...
  if ((job.status as string) === "ready") {
    return new NextResponse("Cannot mark a ready job as failed.", { status: 409 });
  }

  await admin
    .from("generation_jobs")
    .update({ status: "failed", error_message: "Marked failed by admin." })
    .eq("id", id);
  logger.warn("admin_mark_failed", { adminEmail: auth.user.email, jobId: id });
